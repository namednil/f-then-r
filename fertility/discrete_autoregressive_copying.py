...
@Model.register("discrete_autoregressive_copy")
class DiscreteAutoregressiveTransductionWithCopy(Model):
    ...
    def __init__(self, vocab: Vocabulary,
                 ...
                 closed_vocabulary_grammar: bool = True,
        ...
        if closed_vocabulary_grammar and grammar is not None:
            ...
            raise NotImplementedError("Please provide a grammar for now to enable copying.")
    ...
    def compute_logits(self, encoder_outputs: torch.Tensor, embedded_source: torch.Tensor, source_mask: torch.Tensor,
        ...
        if source_to_copy_mask is not None:
            anti_source_to_copy_mask = (1 - source_to_copy_mask) * source_mask.unsqueeze(
                2) * target_mask.unsqueeze(1)
        ...
        for o in range(max_output_seq_len):
            ...
            orig_joint_probs = joint_probs

            current_alignment = alignment_after_fertility_and_reorder[:, :, o, :] #shape (batch_size, input_seq_len, max fertility + 1)
            # Create mask for copy symbol
            copy_part = torch.zeros((1, self.vocab.get_vocab_size(namespace=self.target_namespace)),
                                    dtype=torch.bool,
                                    device=joint_probs.device)  # shape ("batch", vocab), where "batch" and "output" are set to 1.
            copy_part[:, self.copy_id] = True

            # Separate probability for copying from outputting proper vocab tokens
            copy_probs = joint_probs[:, :, :, self.copy_id]  # shape (batch_size, input_seq_len, max fertility)


            if source_to_copy_mask is not None:
                joint_probs = joint_probs * ~copy_part.unsqueeze(
                    1).unsqueeze(2)  # sets probability for generating copy symbol (from vocabulary) to 0

                # we are at training time, only certain inputs are eligible for copying

                current_source_to_copy_mask = source_to_copy_mask[:, :, o]
                current_anti_source_to_copy_mask = anti_source_to_copy_mask[:, :, o]

                # there might be multiple ways to copy correctly, marginalize over them.
                correct_copy_probs = torch.einsum("bik, bik, bi -> b", copy_probs,
                                                  current_alignment, current_source_to_copy_mask)
                # probabilities of copying in an incorrect way
                incorrect_copy_probs = torch.einsum("bik, bik, bi -> b", copy_probs,
                                                    current_alignment, current_anti_source_to_copy_mask)
                ...
                output_prob = copy_part * correct_copy_probs.unsqueeze(1) + \
                               oov_part * incorrect_copy_probs.unsqueeze(1) + \
                               torch.einsum("bikv, bik -> bv", joint_probs,
                                            current_alignment)
                ...
                if source2vocab is not None:
                    ...
                    # this convenient and useful for grammar-based decoding.
                    joint_probs = joint_probs * ~copy_part.unsqueeze(
                        1).unsqueeze(2)  # sets probability for generating copy symbol (from vocabulary) to 0

                    output_prob = torch.einsum("bik, bik, biv -> bv", copy_probs,
                                                current_alignment, source2vocab) \
                                   + torch.einsum("bikv, bik -> bv", joint_probs,
                                                  current_alignment)
                else:
            ...
            if target_tokens is not None:
                ...
                # Greedy
                # We want to give the model the copy embedding, if copying is the most likely course of action.

                output_prob_wo_copy = torch.einsum("bikv, bik -> bv", orig_joint_probs, current_alignment)
                embedded_last_output = self.embedding_weight(torch.argmax(output_prob_wo_copy, dim=-1))
    ...
    def most_likely_sequence(self, encoder_outputs, embedded_source, source_mask, k: int, source2vocab: Optional[torch.Tensor] = None) -> Tuple[
    ...
    def forward(self, source_tokens: TextFieldTensors,
        ...
        if not self.training:
            ...
            predictions, predicted_lengths, debug_info = self.most_likely_sequence(encoder_outputs, embedded_source,
                                                                                   source_mask, self.k, source2vocab=source2vocab)
        ...
        if target_tokens and (self.training or self.compute_loss_during_dev):
            ...
            output_logits, d = self.compute_logits(encoder_outputs, embedded_source, source_mask, target_mask, targets, source_to_copy_mask=source_to_copy_mask)
            for k, v in d.items():
                if k != "output":
